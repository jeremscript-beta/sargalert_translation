...
nc_file = fullfile(current_dir, 'cmems_mod_glo_phy-cur_anfc_0.083deg_PT6H-i_1721738266405.nc');
...
for i = 1:length(mat_files)
    ...
    plot(data.lonim, data.latim, 'DisplayName', ['Observed ID: ', mat_files(i).name]);
    ...
    plot(synthetic_lon, synthetic_lat, '--', 'DisplayName', ['Synthetic ID: ', mat_files(i).name]);

    % Save the synthetic trajectory to a .mat file
    latim = synthetic_lat;
    lonim = synthetic_lon;
    [~, name, ~] = fileparts(mat_files(i).name);
    new_name = ['drifter_bkg_' name(end-1:end)]; % Format name to drifter_truth_XX
